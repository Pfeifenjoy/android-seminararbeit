...
Um ein Bild zu löschen muss der sowohl das File, in dem das Bild gespeichert ist gelöscht
...
\begin{tikzpicture}
    \node(liste) {Liste};
    \node(adapter)[right= of liste] {ImageItemAdapter};
    \node(handler)[right= of adapter] {PiccerDatabaseHandler};
    \node(database)[right= of handler] {Database};
    \node(loader)[below= of handler] {ImageThumbnailLoader};
    \node(cache)[right= of loader] {Cache};
    \node(file)[right= of cache] {Filesystem};

    \path
    (liste) edge[post] (adapter)
    (adapter) edge[post] (handler)
    ;
\end{tikzpicture}
